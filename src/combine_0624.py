...
# -*- coding: utf-8 -*-
"""
dispatch-only Batch wrapper
===========================

• 對指定 (layer, neuron) 組合掃描 Segments (4–12) × ContainerCount (4–12)。
• 每組 (S,C) 產生：
...
------------------
None
    → 掃描所有 layer、所有 neuron（預設）。
list[int]
    → 僅掃描列出的 layer，且「該 layer 的所有 neuron」全跑。
dict[int, None | int | list[int]]
    → 逐層指定要跑哪些 neuron：
        {4: None}        → layer 4 全掃 neuron
        {4: 7}           → layer 4 只跑 neuron 7
        {4: [3,7,15]}    → layer 4 跑 3,7,15
        {4: [7], 5: None}→ layer 4 跑 7；layer 5 全掃；其他 layer 不跑
"""
...
OUTPUT_BASE_DEFAULT  = "archive/test1"

SEG_MIN, SEG_MAX     = 15, 16
CONT_MIN, CONT_MAX   = 15, 16

USE_SEQUENTIAL_INPUT = True      #True = 依序；False = 交錯
EXCLUDE_ZERO_WEIGHT  = True       #weight=0不分配通道
USE_AUTO_QUOTA       = True       #通道分配依照weight比例
...
LAYERS_TO_RUN        = {0: 0}
# 例:
# LAYERS_TO_RUN = [4]                         # layer 4 全掃
# LAYERS_TO_RUN = {4: [7], 5: None}           # 逐層指定
# LAYERS_TO_RUN = {4: 7}                      # layer 4 neuron 7

TXT_REARRANGE        = "rearrange_detail.txt"
TXT_ENACC_DEC        = "en_acc_group_view.txt"
TXT_IDX_MEM          = "channel_idx_mem.mif"
TXT_ENACC_MEM        = "en_acc_mem.mif"
OUTPUT_CSV           = "dispatch_simulation.csv"
...
def get_neuron_list(total, layer_idx):
    """
    根據全域 LAYERS_TO_RUN 回傳本 layer 要跑哪些 neuron index。
    傳回 [] 代表本層整個 skip。
    """
    spec = LAYERS_TO_RUN
    if spec is None:
    ...
    if isinstance(spec, list):
        return list(range(total)) if layer_idx in spec else []

    # ───── 原本就是這段 ────────────────────────────────────────────
    if isinstance(spec, dict):
        # ★ 新增：若 dict 內含萬用字元 "*"，先展開成對所有 layer 生效的條目
        if "*" in spec:
            wildcard = spec["*"]                 # 可能是 None / int / list
            # 若當前 layer 沒有被明確指定，才套用 wildcard
            if layer_idx not in spec:
                layer_spec = wildcard
            else:
                layer_spec = spec[layer_idx]
        else:
            if layer_idx not in spec:
                return []
            layer_spec = spec[layer_idx]
        ...
        if isinstance(layer_spec, int):
            return [layer_spec] if 0 <= layer_spec < total else []
        if isinstance(layer_spec, list):
            return [n for n in layer_spec if 0 <= n < total]

    raise ValueError("LAYERS_TO_RUN format not supported")
...
def run_single(base_dir, model, layer_idx, neuron_idx, n_seg, cont_cnt):
    ...
    each_file = os.path.join(out_dir, prefix + "each_channel_weight.mif")
...
def main():
    ...
    elif isinstance(LAYERS_TO_RUN, dict):
        if "*" in LAYERS_TO_RUN:
            layers = layers_all
        else:
            layers = [l for l in layers_all if l in LAYERS_TO_RUN]
    ...
    for L in layers:
        ...
        neuron_cnt = len(get_neuron_list(weights_shape[0], L))
        total_tasks += neuron_cnt * (SEG_MAX - SEG_MIN + 1) * (CONT_MAX - CONT_MIN + 1)
    ...
    for L in (tqdm(layers, desc="Layers") if _tqdm_available else layers):
        ...
        neuron_list = get_neuron_list(n_neurons, L)
        if not neuron_list:
            print(f"[WARNING] Skip layer {L} (no neuron selected).")
            continue

        neuron_iter = (tqdm(neuron_list, desc=f"Layer {L} Neurons", leave=False)
                       if _tqdm_available else neuron_list)
